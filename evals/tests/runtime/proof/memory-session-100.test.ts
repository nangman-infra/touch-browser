...
const scenarioRoot = `${scenarioFixturesRoot}/memory-100-step`;

describe("memory 100-step baseline", () => {
  it("keeps working memory bounded across the 100-step generated session", async () => {
    ...
    expect(summary.requestedActions).toBe(100);
    expect(summary.actionCount).toBe(100);
    expect(summary.memorySummary.turnCount).toBe(100);
    ...
    expect(summary.sessionState.snapshotIds.length).toBe(50);
