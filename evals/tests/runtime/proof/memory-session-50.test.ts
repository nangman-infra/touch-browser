...
const scenarioRoot = `${scenarioFixturesRoot}/memory-50-step`;

describe("memory 50-step baseline", () => {
  it("keeps working memory bounded across the longer generated session", async () => {
    ...
    expect(summary.requestedActions).toBe(50);
    expect(summary.actionCount).toBe(50);
    expect(summary.memorySummary.turnCount).toBe(50);
    ...
    expect(summary.sessionState.snapshotIds.length).toBe(25);
