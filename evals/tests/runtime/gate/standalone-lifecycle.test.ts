...
describe("standalone lifecycle smoke", () => {
  it("installs, updates, and uninstalls the managed standalone command", async () => {
    ...
    try {
      ...
      expect(versionText).toBe("touch-browser 1.5.5");
      ...
      const preflight = JSON.parse(
        await runShell(
          `${shellQuote(commandPath)} update --check --version 1.5.6`,
          env,
        ),
      );
      expect(preflight.currentVersion).toBe("v1.5.5");
      expect(preflight.targetVersion).toBe("v1.5.6");
      expect(preflight.updateAvailable).toBe(true);

      await runShell(`${shellQuote(commandPath)} update --version 1.5.6`, env);
      ...
      expect(installManifest.version).toBe("v1.5.6");
      ...
      expect(postUpdate.currentVersion).toBe("v1.5.6");
      expect(postUpdate.targetVersion).toBe("v1.5.6");
