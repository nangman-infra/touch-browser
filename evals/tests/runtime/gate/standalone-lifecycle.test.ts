...
describe("standalone lifecycle smoke", () => {
  it("installs, updates, and uninstalls the managed standalone command", async () => {
    ...
    try {
      ...
      expect(versionText).toBe("touch-browser 1.5.2");
      ...
      const preflight = JSON.parse(
        await runShell(
          `${shellQuote(commandPath)} update --check --version 1.5.3`,
          env,
        ),
      );
      expect(preflight.currentVersion).toBe("v1.5.2");
      expect(preflight.targetVersion).toBe("v1.5.3");
      expect(preflight.updateAvailable).toBe(true);

      await runShell(`${shellQuote(commandPath)} update --version 1.5.3`, env);
      ...
      expect(installManifest.version).toBe("v1.5.3");
      ...
      expect(postUpdate.currentVersion).toBe("v1.5.3");
      expect(postUpdate.targetVersion).toBe("v1.5.3");
